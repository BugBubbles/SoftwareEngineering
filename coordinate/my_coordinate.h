...
#include "based_geometry.h"
  ...
  coordinatev_t new_top_coor(float min_x, float max_x, float min_y, float max_y); // 创建绝对坐标系
  coordinatev_t new_sub_coor(float base_point_x, float base_point_y,
                             float min_x, float max_x, float min_y, float max_y,
                             coordinatec_t *parent_coor);                                            // 创建一个子坐标系
  point_t relati_point_to_absol_point(coordinatec_t relati_coor, point_t relati_point);              // 把相对坐标系的点转换为绝对坐标系下的点
